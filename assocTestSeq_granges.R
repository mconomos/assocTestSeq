assocTestSeq <- function(	seqData,
	...
	param <- .paramChecks(AF.range = AF.range, weight.type = weight.type, test = test, burden.test = burden.test, 
							family = nullModObj$family$family, mixedmodel = nullModObj$family$mixedmodel, rho = rho, pval.method = pval.method)
	...
	if(test == "Burden"){
		...
		if(verbose) message("Performing ", burden.test, " Burden Tests for Variants with AF in [", AF.range[1], ",", AF.range[2], "] using ", weight.type, " weights")

	}else if(test == "SKAT"){
		nv <- append(nv, c(paste("Q",rho,sep="_"), paste("pval",rho,sep="_"), paste("err",rho,sep="_")))
		if(length(rho) == 1){			
			if(verbose) message("Performing SKAT Tests for Variants with AF in [", AF.range[1], ",", AF.range[2], "] using ", weight.type, " weights and rho = ", rho)
		}else{
			nv <- append(nv, c("min.pval", "opt.rho", "pval_SKATO"))
			if(verbose) message("Performing SKAT-O Tests for Variants with AF in [", AF.range[1], ",", AF.range[2], "] using ", weight.type, " weights and rho = (", paste(rho, collapse=", "), ")")
		}		
	}
	...
	for(b in 1:nblocks){		
		...
		freq <- alleleFreq(geno = geno, chromChar = chromChar, sex = NULL)
		...
		if(n.site == 0){	
			...
			# variant weights
			if(weight.type == "MB"){
				weights <- (freq*(1-freq))^(-0.5) # Madsen Browning
			}else if(weight.type == "Wu"){
				# use MAF for Beta parameter
				freq <- ifelse(freq < 0.5, freq, 1-freq)
				weights <- dbeta(freq, 1, 25) # Wu weights
			}else if(weight.type == "Equal"){
				weights <- rep(1, length(freq))
			}
...
assocTestSeqWindow <- function(	seqData,
	...
	out[["window.size"]] <- window.size
	out[["window.shift"]] <- window.shift
	...
	for(chr in unique(variant.chr)){
		...
		# all variants in GDS file as GRanges
		gr <- granges(gds)		
		# set up windows
		window.start <- seq(from = (ceiling(min(start(gr))/(window.shift*1000))-1)*window.shift*1000 + 1, 
							to = max(end(gr)) - window.size*1000 + window.shift*1000, 
							by = window.shift*1000)		
		windows <- GRangesList(lapply(window.start, function(i){ GRanges(seqnames = chr, ranges = IRanges(start = i, width = window.size*1000)) }))		
		# only keep windows that contain variants
		windows <- subsetByOverlaps(windows, gr)
		nblocks <- length(windows)

		## find variants in each window - contains the indices of variants in gr that fall in each window
		variant.window <- lapply(windows, function(x) queryHits(findOverlaps(gr, x)))
		
		# results for this chromosome
		resChr <- matrix(NA, nrow = nblocks, ncol = length(nv), dimnames = list(NULL,nv))
		resChr[,"chr"] <- as.numeric(chr)
		resChr[,"window.start"] <- unlist(start(windows))
		resChr[,"window.stop"] <- unlist(end(windows))
		...
		for(b in 1:nblocks){
			...
			windowID <- variantID[variant.window[[b]]]
			...
			if(length(var.add) > 0){
				...
				# calculate allele frequencies
				freq <- alleleFreq(geno = geno.add, chromChar = chromChar, sex = NULL)
				variantRes[,"freq"] <- freq
				...
				# variant weights
				if(weight.type == "MB"){
					weights.add <- (freq*(1-freq))^(-0.5) # Madsen Browning
				}else if(weight.type == "Wu"){
					# use MAF for Beta parameter
					freq <- ifelse(freq < 0.5, freq, 1-freq)
					weights.add <- dbeta(freq, 1, 25) # Wu weights
				}else if(weight.type == "Equal"){
					weights.add <- rep(1, length(freq))
				}
			...
			if(n.site > 0){
				...
				# update main results
				for(val in names(testout)){ resChr[b,val] <- testout[val] }
			}
...
.paramChecks <- function(AF.range, weight.type, test, burden.test, family, mixedmodel, rho, pval.method){
	# list of parameters
	param <- list()
	param[["AF.range"]] <- AF.range
	param[["weight.type"]] <- weight.type
	param[["test"]] <- test
	
	# check AF.range
	if(AF.range[1] < 0 | AF.range[2] > 1){ stop("AF.range must be in [0,1]")}
	# check weight.type
	if(!(weight.type %in% c("MB","Wu","Equal"))){ stop("weight.type must be one of 'MB', 'Wu', or 'Equal'")}
